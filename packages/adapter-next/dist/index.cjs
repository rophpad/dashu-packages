...
async function resolveActor(request, options) {
  ...
  if (!actor) {
    throw new dashuCore.DashuError("FORBIDDEN", "Administrator access is required.");
  }
  return actor;
}
function fail(error) {
  const requestId = error instanceof dashuCore.DashuError && error.requestId ? error.requestId : "req_unknown";
  if (error instanceof dashuCore.DashuError) {
    if (error.detail) console.error(`[dashu] ${requestId} ${error.code}: ${error.detail}`);
  } else {
    console.error(`[dashu] ${requestId} unhandled`, error);
  }
  return json(dashuCore.toErrorResponse(error, requestId), dashuCore.errorStatus(error));
}
  ...
  return async function POST(request) {
    try {
      ...
      try {
        ...
        throw new dashuCore.DashuError("INVALID_REQUEST", "Invalid JSON body.");
      }
...
exports.dashuRoute = dashuRoute;
exports.dashuRunRoute = dashuRunRoute;
exports.dashuSchemaRoute = dashuSchemaRoute;
